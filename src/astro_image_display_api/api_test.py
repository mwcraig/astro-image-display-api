...
class ImageAPITest:
    ...
    # Tests of the template-method hooks: the public API methods own all
    # state handling and label resolution, then call private no-op hooks
    # (with already-resolved labels) that backends override to update
    # their display.

    def _make_recording_viewer(self):
        """
    ...
    def test_catalog_hooks_get_resolved_labels(self, catalog):
        ...
        calls.clear()
        viewer.set_catalog_style(
            catalog_label="cat", color="blue", shape="square", size=5
        )
    ...
    def test_load_image_carries_forward_display_settings(self, data):
        ...
        self.image.set_cuts((10, 100), image_label="first")
        self.image.set_stretch(LogStretch(), image_label="first")
        self.image.set_colormap("viridis", image_label="first")

        self.image.load_image(data * 2, image_label="second")

        assert self.image.get_cuts(image_label="second").get_limits(data) == (10, 100)
        assert isinstance(self.image.get_stretch(image_label="second"), LogStretch)
        ...
        self.image.set_cuts((1, 2), image_label="second")
        assert self.image.get_cuts(image_label="first").get_limits(data) == (10, 100)

    def test_load_image_carry_forward_uses_displayed_image(self, data):
        # The settings carried forward must come from the image being
        # displayed, not from some other loaded image.
        self.image.load_image(data, image_label="first")
        self.image.load_image(data, image_label="second")  # now displayed

        self.image.set_colormap("plasma", image_label="first")
        self.image.set_colormap("viridis", image_label="second")

        self.image.load_image(data, image_label="third")
        assert self.image.get_colormap(image_label="third") == "viridis"
